...
import { 
  ...
  Eye, Grid, BookOpen, Coffee, Flame, Star, Zap
} from 'lucide-react';
...
const MONTHS_ES = [
  {id:'abril',n:'Abril',num:4,phase:'lanzamiento'},
...
const STATUS = {
  pending: {label:'Pendiente programar', color:C.pending, icon:Circle},
  creating: {label:'Creando / en proceso', color:C.creating, icon:PauseCircle},
...
function effectiveStatus(post, platform, state, now) {
  ...
  if (rawStatus !== 'scheduled') return rawStatus;
  // Si está programado, mirar si ya pasó su hora
  ...
  if (now >= postTime) return 'published';
  return rawStatus;
}
...
export default function App() {
  ...
  const groups = ['principal','redes','recursos','análisis'];
...
function Dashboard({setView, month, setMonth}) {
  ...
  const allPosts = [
    ...D.posts.ig.map(p => ({...p, platform:'instagram'})),
    ...D.posts.li.map(p => ({...p, platform:'linkedin'})),
    ...D.posts.ss.map(p => ({...p, platform:'substack'})),
  ];
              ...
              {todayTiktoks.length > 0 && (
                <div style={{background:C.card, borderRadius:12, padding:14}}>
                  ...
                  {todayTiktoks.map((p, i) => (
                    <div key={i} style={{
                      fontSize:12, color:C.ink, padding:'6px 0', lineHeight:1.4,
...
function PostCard({post, platform, onEdit, onDelete}) {
  ...
  const cycleStatus = () => {
    const order = ['pending','creating','scheduled','published'];
...
function CalendarView({month, setMonth}) {
  ...
  const collectPosts = () => {
    ...
    D.posts.li.filter(p => p.m === month).forEach(p => {
      if (!p.d) return;
      const day = parseInt(p.d.split('-')[2]);
      if (day && day <= daysInMonth) out.push({...p, pf: 'linkedin', day});
    });
    D.posts.ss.filter(p => p.m === month).forEach(p => {
      if (!p.d) return;
      const day = parseInt(p.d.split('-')[2]);
              ...
              <div key={i} style={{
                ...
                <div style={{display:'flex', flexDirection:'column', gap:3, minWidth:0}}>
                  {posts.slice(0, 4).map((p, j) => {
                    ...
                    // FONDO = SIEMPRE color de plataforma (identidad visual de la red)
                    // BORDE IZQUIERDO = estado (verde publicado, amarillo proceso, gris pendiente)
                    const statusBorderColor = 
                      isDone ? C.published : 
                      isCreating ? C.creating : 
                      isScheduled ? '#7BA2C2' : 
                      'rgba(255,255,255,0.4)';
                    
                    return (
                      <div key={j} title={`${pf.name} · ${p.tm || ''} · ${p.ti || p.t || ''} · ${STATUS[s]?.label || s}`} style={{
...
function PlatformView({platform, month, setMonth}) {
  ...
  const handleAddPost = (saved) => {
    update(s => {
      ...
      // Check if editing existing custom post
        ...
        Object.keys(byWeek).sort((a,b) => +a - +b).map(w => (
          <div key={w} style={{marginBottom:24}}>
            <div style={{fontSize:10, color:C.inkSoft, textTransform:'uppercase', letterSpacing:2, fontWeight:600, marginBottom:10, paddingLeft:4}}>
              semana {w} {byWeek[w][0]?.f ? '· ' + byWeek[w][0].f : ''}
            </div>
            {byWeek[w].map(p => (
...
const WEEKLY_DEFAULTS = {
  ...
  '2026-04-26': {
    ...
    ss_subscribers: 4,    // 4 emails dijo Irene
      ...
      <Card style={{marginBottom:20}}>
        <div style={{display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:14, flexWrap:'wrap', gap:10}}>
          <div>
            ...
            <H size="md" style={{fontStyle:'italic'}}>próximos {contentRange} días ♡</H>
          </div>
          <Badge bg={C.igBg} color={C.igDark} style={{fontSize:11}}>
            {upcomingPosts.filter(p => getPostStatus(p) !== 'published' && getPostStatus(p) !== 'scheduled').length} por crear
          </Badge>
        </div>

        {/* Filtro de rango 7 · 15 · 30 */}
        ...
        {upcomingPosts.length === 0 ? (
          <div style={{padding:'24px 0', textAlign:'center', fontSize:13, color:C.inkSoft, fontStyle:'italic'}}>
            no hay contenido programado en los próximos {contentRange} días ✧
          </div>
        ) : upcomingPosts.filter(p => getPostStatus(p) !== 'published').map((p, i, visibleArr) => {
          const pf = PLATFORMS[p.pf];
          const status = getPostStatus(p);
          const isCreating = status === 'creating';
          const isScheduled = status === 'scheduled';

          // Dónde se programa según plataforma
          const schedulingPlatform = 
            p.pf === 'instagram' ? 'Meta Business Suite' :
            p.pf === 'linkedin' ? 'LinkedIn (publicación programada)' :
            p.pf === 'substack' ? 'Substack (programación directa)' : '';

          return (
            <div key={i} style={{
              display:'flex', alignItems:'flex-start', gap:12, padding:'12px',
              background: isScheduled ? '#F0F6FA' : C.card,
              borderRadius:10, marginBottom:6,
              border:`1px solid ${isScheduled ? '#C0D8EB80' : C.border}`,
              borderLeft: `3px solid ${pf.dark}`
            }}>
              {/* Checkbox principal · al marcar pasa a scheduled (programado) */}
              <button onClick={() => setPostStatus(p, isScheduled ? 'pending' : 'scheduled')} style={{
                ...
                marginTop:2
              }} title="marcar como programado (desaparece de la lista)">
                {isScheduled ? '✓' : ''}
              </button>

              <div style={{
                minWidth:48, textAlign:'center', flexShrink:0,
                padding:'6px 0', background:pf.bg, borderRadius:8, color:pf.dark
              }}>
                <div style={{fontFamily:"'Fraunces', serif", fontSize:14, fontWeight:500, lineHeight:1}}>
                  {p.dt.getDate()}
                </div>
                <div style={{fontSize:9, textTransform:'uppercase', letterSpacing:0.5, marginTop:2}}>
                  {mesesShort[p.dt.getMonth()]}
                </div>
              </div>

              <div style={{flex:1, minWidth:0}}>
                <div style={{display:'flex', gap:6, alignItems:'center', marginBottom:4, flexWrap:'wrap'}}>
                  <Badge bg={pf.bg} color={pf.dark} style={{fontSize:10}}>{pf.name}</Badge>
                  {p.t && <Badge bg={C.bgSoft} color={C.inkSoft} style={{fontSize:10}}>{p.t}</Badge>}
                  <span style={{fontSize:11, color:C.inkSoft}}>
                    {p.tm || '—'} · {daysUntil(p.dt)}
                  </span>
                  {isCreating && <span style={{fontSize:10, color:C.creating, fontWeight:600, fontStyle:'italic'}}>◐ en proceso</span>}
                </div>
                <div style={{
                  fontSize:13, color:C.ink, lineHeight:1.35, fontWeight:500, marginBottom:6
                }}>
                  {p.ti || p.su}
                </div>
                <div style={{fontSize:10, color:C.inkSoft, fontStyle:'italic'}}>
                  📱 programa en: <strong style={{color:C.ink, fontWeight:500}}>{schedulingPlatform}</strong>
                </div>
              </div>

              <div style={{display:'flex', gap:4, flexShrink:0}}>
                <button onClick={() => setPostStatus(p, isCreating ? 'pending' : 'creating')}
                  title={isCreating ? 'quitar en proceso' : 'marcar en proceso'}
                  style={{
                    width:28, height:28, borderRadius:8, border:'none', cursor:'pointer',
                    background: isCreating ? C.creating : C.bgSoft,
                    fontSize:14, color:isCreating ? '#fff' : C.inkSoft, fontFamily:'inherit'
                  }}>◐</button>
              </div>
            </div>
          );
        ...
        <div style={{fontSize:11, color:C.inkSoft, marginTop:10, fontStyle:'italic', lineHeight:1.5, paddingTop:10, borderTop:`1px solid ${C.borderSoft}`}}>
          ✓ marcar como programado · ◐ en proceso · al marcar programado, desaparece de esta lista y se publica sola cuando llegue su hora ♡
        </div>

        {/* Ya programados · plegable · con opción de deshacer si marcaste sin querer */}
        {(() => {
          const scheduledOnes = upcomingPosts.filter(p => getPostStatus(p) === 'scheduled');
          if (scheduledOnes.length === 0) return null;
          return (
            <details style={{marginTop:14, paddingTop:14, borderTop:`1px solid ${C.borderSoft}`}}>
              <summary style={{
                cursor:'pointer', fontSize:11, color:C.inkSoft, fontWeight:600,
                textTransform:'uppercase', letterSpacing:1, padding:'4px 0',
                listStyle:'none', userSelect:'none'
              }}>
                ↺ ya programados ({scheduledOnes.length}) · clic para ver y deshacer si te equivocaste
              </summary>
              <div style={{marginTop:10, display:'flex', flexDirection:'column', gap:4}}>
                {scheduledOnes.map((p, i) => {
                  const pf = PLATFORMS[p.pf];
                  return (
                    <div key={i} style={{
                      display:'flex', alignItems:'center', gap:10, padding:'8px 10px',
                      background: '#F0F6FA', borderRadius:8,
                      border:`1px solid #C0D8EB60`
                    }}>
                      <span style={{
                        fontSize:10, padding:'2px 7px', background:pf.bg, color:pf.dark, 
                        borderRadius:100, fontWeight:500, flexShrink:0
                      }}>{pf.name}</span>
                      <span style={{fontSize:11, color:C.inkSoft, flexShrink:0}}>
                        {p.dt.getDate()} {mesesShort[p.dt.getMonth()]} · {p.tm}
                      </span>
                      <span style={{flex:1, fontSize:12, color:C.ink, lineHeight:1.3, minWidth:0, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap'}}>
                        {p.ti || p.su}
                      </span>
                      <button onClick={() => setPostStatus(p, 'pending')} title="volver a por crear" style={{
                        padding:'4px 10px', border:`1px solid ${C.border}`, background:'transparent',
                        color:C.inkSoft, borderRadius:100, fontSize:10, cursor:'pointer', 
                ...
                <div style={{padding:'0 14px 14px 14px'}}>
                  <div style={{borderTop:`1px solid ${C.borderSoft}`, paddingTop:12, display:'flex', flexDirection:'column', gap:6}}>
                    {checklist.map((task, idx) => {
                      ...
                      const isToday = Math.abs(deadline - now) < 86400000;
                      ...
                      return (
                        <div key={idx} onClick={() => toggleLaunchCheck(launch.id, idx)} style={{
                          display:'flex', alignItems:'flex-start', gap:10, padding:'9px 11px',
                          background: isChecked ? C.card : (isPast ? '#FFF1F1' : C.card),
                          borderRadius:8, cursor:'pointer',
                          border: `1px solid ${isPast ? '#F0C8C8' : C.borderSoft}`
                        }}>
                          <div style={{
                            width:20, height:20, borderRadius:10, flexShrink:0,
                          ...
                          <div style={{flex:1, minWidth:0}}>
                            ...
                            <div style={{fontSize:10, color: isPast ? '#B04545' : C.inkSoft, marginTop:3, fontStyle:'italic'}}>
...
function BackupBox({compact = false}) {
  ...
  const handleExport = () => {
    exportBackup(state);
    // Forzar re-render con un timeout
  ...
  if (compact) {
    return (
      <div style={{
        ...
        background: needsBackup ? '#FFF1E8' : '#F0F8F0',
        borderRadius:10, border: needsBackup ? '1px solid #F0C8A8' : '1px solid #C5D9C0'
      }}>
        <span style={{fontSize:14}}>{needsBackup ? '⚠️' : '♡'}</span>
        <div style={{flex:1, minWidth:0}}>
          <div style={{fontSize:12, color:C.ink, fontWeight:500}}>
            {needsBackup 
              ? (lastBackup ? `${daysSinceBackup} días sin backup` : 'sin backup aún')
              : `último backup: hace ${daysSinceBackup} ${daysSinceBackup === 1 ? 'día' : 'días'}`}
          </div>
          <div style={{fontSize:10, color:C.inkSoft, fontStyle:'italic'}}>
            haz un backup ahora si has marcado cosas o metido métricas hoy
          </div>
        </div>
