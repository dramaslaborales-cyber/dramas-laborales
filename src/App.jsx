...
const _CM_DAYS = ['dom','lun','mar','mié','jue','vie','sáb'];
const _CM_MOS  = ['ene','feb','mar','abr','may','jun','jul','ago','sep','oct','nov','dic'];

function _cmNormalize(p) {
  if(!p.d) return p;
  const dt = new Date(p.d+'T12:00:00');
  return {
    ...p,
    day: (p.day && p.day.length>1) ? p.day : _CM_DAYS[dt.getDay()],
    ds: (p.ds && /^[0-9]/.test(p.ds)) ? p.ds : dt.getDate()+' '+_CM_MOS[dt.getMonth()],
  };
}

const CM_ASESORIA_TASKS = [
  {id:'a1', cat:'definición', label:'Finalizar pitch de venta · 2 perfiles', due:'22 may'},
  {id:'a2', cat:'definición', label:'Script sesión 2h30 · Principiante', due:'25 may'},
  {id:'a3', cat:'definición', label:'Script sesión 2h30 · Experta Estancada', due:'27 may'},
  {id:'a4', cat:'entregables', label:'Plantilla CV · Principiante', due:'29 may'},
  {id:'a5', cat:'entregables', label:'Plantilla CV · Experta Estancada', due:'30 may'},
  {id:'a6', cat:'entregables', label:'Plantilla LinkedIn (encabezado, resumen, experiencia)', due:'31 may'},
  {id:'a7', cat:'entregables', label:'Plantilla propuesta de valor + pitch personal', due:'1 jun'},
  {id:'a8', cat:'entregables', label:'Plantilla mini-plan de acción 2-4 semanas', due:'2 jun'},
  {id:'a9', cat:'entregables', label:'Plantilla informe competencias transferibles (Experta)', due:'3 jun'},
  {id:'a10', cat:'entregables', label:'Ejercicios role-play entrevistas (ambos perfiles)', due:'4 jun'},
  {id:'a11', cat:'infra', label:'Zoom cloud o Loom para grabar sesiones', due:'6 jun'},
  {id:'a12', cat:'infra', label:'Sistema de reservas (Calendly / Cal.com)', due:'8 jun'},
  {id:'a13', cat:'infra', label:'Pasarela de pago Stripe · 90€', due:'10 jun'},
  {id:'a14', cat:'infra', label:'Email bienvenida + cuestionario previo', due:'12 jun'},
  {id:'a15', cat:'infra', label:'Email confirmación de cita + instrucciones', due:'14 jun'},
  {id:'a16', cat:'infra', label:'Email post-sesión con entregables', due:'16 jun'},
  {id:'a17', cat:'venta', label:'Copy página de venta completa', due:'18 jun'},
  {id:'a18', cat:'venta', label:'Página de venta (2 versiones: Principiante + Experta)', due:'20 jun'},
  {id:'a19', cat:'venta', label:'Imagen promocional en Canva', due:'22 jun'},
  {id:'a20', cat:'venta', label:'FAQ · añadirla a página de venta', due:'24 jun'},
  {id:'a21', cat:'contenido', label:'Reel teaser "algunas necesitan más que un ebook"', due:'22 jun'},
  {id:'a22', cat:'contenido', label:'Newsletter Substack pre-launch', due:'26 jun'},
  {id:'a23', cat:'contenido', label:'Reel lanzamiento + carrusel "qué incluye"', due:'29 jun'},
  {id:'a24', cat:'contenido', label:'Post LinkedIn + copies Instagram lanzamiento', due:'1 jul'},
  {id:'a25', cat:'contenido', label:'Guion stories de lanzamiento', due:'3 jul'},
  {id:'a26', cat:'test', label:'Probar flujo: compra → reserva → email → Zoom', due:'4 jul'},
  {id:'a27', cat:'test', label:'Revisar plantillas finales y carpetas', due:'5 jul'},
  {id:'a28', cat:'launch', label:'♡ LANZAR · reel 09:00 + stories + email a lista', due:'6 jul'},
];

const CM_ACOMP_TASKS = [
  {id:'c1', cat:'definición', label:'Pitch de venta · 2 perfiles', due:'21 ago'},
  {id:'c2', cat:'definición', label:'Script sesión 1 Diagnóstico · Principiante', due:'24 ago'},
  {id:'c3', cat:'definición', label:'Script sesión 2 Fortalezas · Principiante', due:'26 ago'},
  {id:'c4', cat:'definición', label:'Script sesión 3 Estrategia búsqueda · Principiante', due:'28 ago'},
  {id:'c5', cat:'definición', label:'Script sesión 4 Comunicación · Principiante', due:'30 ago'},
  {id:'c6', cat:'definición', label:'Scripts sesiones 1-4 · Experta Estancada', due:'1 sep'},
  {id:'c7', cat:'definición', label:'Worksheets y ejercicios entre sesiones', due:'3 sep'},
  {id:'c8', cat:'entregables', label:'Plantilla plan de acción por sesión', due:'5 sep'},
  {id:'c9', cat:'entregables', label:'Plantilla CV + LinkedIn + carta presentación', due:'7 sep'},
  {id:'c10', cat:'entregables', label:'Plantilla plan de continuidad 3 meses', due:'9 sep'},
  {id:'c11', cat:'entregables', label:'Material simulación entrevistas (por perfil)', due:'11 sep'},
  {id:'c12', cat:'infra', label:'Zoom cloud + canal comunicación entre sesiones', due:'15 sep'},
  {id:'c13', cat:'infra', label:'Definir acompañamiento entre sesiones', due:'18 sep'},
  {id:'c14', cat:'infra', label:'Página de venta 625€ + términos', due:'20 sep'},
  {id:'c15', cat:'infra', label:'Contrato / carta de compromiso', due:'22 sep'},
  {id:'c16', cat:'infra', label:'Stripe: pago único o 2 plazos', due:'24 sep'},
  {id:'c17', cat:'infra', label:'Serie de emails (bienvenida + pre/post sesión ×4)', due:'26 sep'},
  {id:'c18', cat:'infra', label:'Formulario de postulación (3-5 plazas)', due:'28 sep'},
  {id:'c19', cat:'contenido', label:'Materiales visuales worksheets (Canva)', due:'29 sep'},
  {id:'c20', cat:'contenido', label:'Reel teaser "a veces una sesión no es suficiente"', due:'2 oct'},
  {id:'c21', cat:'contenido', label:'Newsletter Substack sobre acompañamientos', due:'6 oct'},
  {id:'c22', cat:'contenido', label:'Lista de espera + form + email confirmación', due:'10 oct'},
  {id:'c23', cat:'contenido', label:'Reel "para quién es" + carrusel diferencias', due:'13 oct'},
  {id:'c24', cat:'contenido', label:'Recopilar testimonios de clientas de asesoría', due:'15 oct'},
  {id:'c25', cat:'contenido', label:'Copy LinkedIn + Instagram + stories de ventas', due:'17 oct'},
  {id:'c26', cat:'launch', label:'Llamadas de selección (3-5 plazas)', due:'18 oct'},
  {id:'c27', cat:'launch', label:'Email final a lista de espera', due:'19 oct'},
  {id:'c28', cat:'launch', label:'♡ LANZAR · reel 09:00 + anunciar plazas', due:'20 oct'},
];

const CM_BF_TASKS = [
  {id:'bf1', cat:'definición', label:'Decidir descuentos: ebook / asesoría / acompañamiento', due:'27 oct'},
  {id:'bf2', cat:'definición', label:'Calcular fecha inicio y fin de la oferta (24-30 nov)', due:'30 oct'},
  {id:'bf3', cat:'contenido',  label:'Diseñar banner Black Friday + variantes feed y stories', due:'3 nov'},
  ...
  {id:'bf6', cat:'contenido',  label:'Diseñar carrusel con todos los productos y precios', due:'19 nov'},
  {id:'bf7', cat:'contenido',  label:'Copies IG + LinkedIn para los 7 días de oferta', due:'21 nov'},
  ...
  {id:'bf11', cat:'launch',    label:'♡ LANZAR BF: carrusel + newsletter + stories todo el día', due:'24 nov'},
];

const CM_ACCIONES = [
  {id:'acc1', pri:'alta', label:'Configurar Inrō "comenta EBOOK" → link al DM', sub:'inro.social · prueba gratuita · 45 min · multiplica conversiones x3-5'},
  {id:'acc2', pri:'alta', label:'Revisar página de venta del ebook', sub:'28 personas hicieron clic, 0 compraron. Precio justificado, bullets concretos, testimonio, CTA claro'},
  {id:'acc3', pri:'alta', label:'Pedir testimonio por DM a compradores del ebook', sub:'Mensaje corto y directo. Una frase honesta cambia todo en mayo'},
  {id:'acc4', pri:'media', label:'Añadir CTA al ebook en última página del checklist', sub:'Canva · 15 minutos · punto de entrada al embudo'},
  {id:'acc5', pri:'media', label:'Escribir y enviar secuencia 3 emails a lista', sub:'Email 1: ejercicio gratis / Email 2: 6 capítulos / Email 3: subida precio'},
  {id:'acc6', pri:'media', label:'Actualizar titular LinkedIn con fórmula correcta', sub:'Qué haces + para quién · sin "buscando oportunidades"'},
  {id:'acc7', pri:'baja',  label:'Cambiar link de bio: checklist primero, ebook segundo', sub:'Linktree · checklist capta emails · ebook convierte · orden importa'},
];
...
function CentroMandoView({month, setMonth}) {
  ...
  const daysUntil = (ds) => {
    ...
    return Math.ceil((new Date(2026, ms[m]||0, +d||1) - now) / 86400000);
  };

  const pfLabel = (pf) => ({instagram:'IG', linkedin:'LI', substack:'SS', tiktok:'TT'})[pf] || 'XX';
  const pfDark  = (pf) => ({instagram:C.igDark, linkedin:C.liDark, substack:C.ssDark, tiktok:C.ttDark})[pf] || C.inkSoft;
  ...
  // all posts (shared state with platform views)
  const deleted = new Set(state.deletedPosts||[]);
  const customPosts = state.customPosts || {};
  const allPosts = [
    ...D.posts.ig.map(p=>({..._cmNormalize(p), pf:'instagram'})),
    ...D.posts.li.map(p=>({..._cmNormalize(p), pf:'linkedin'})),
    ...D.posts.ss.map(p=>({..._cmNormalize(p), pf:'substack'})),
    ...D.posts.ig.filter(p=>/eel/i.test(p.t||'')).map(p=>({..._cmNormalize(p), pf:'tiktok'})),
    ...(customPosts.ig||[]).map(p=>({..._cmNormalize(p), pf:'instagram'})),
    ...(customPosts.li||[]).map(p=>({..._cmNormalize(p), pf:'linkedin'})),
    ...(customPosts.ss||[]).map(p=>({..._cmNormalize(p), pf:'substack'})),
  ].filter(p=>!deleted.has(p.i) && !deleted.has(`${p.pf}:${p.i}`));

  const getStatus = (p) => effectiveStatus(p, p.pf, state, now);
  const setPostStatus = (p, st) => update(s=>{
    const k=`${p.pf}:${p.i}`;
    if(!s.posts[k]) s.posts[k]={};
    s.posts[k].status=st;
  });
  ...
  const toggleLaunchTask = (lid, tid) => update(s=>{
    if(!s.launches) s.launches={};
    if(!s.launches[lid]) s.launches[lid]={};
    s.launches[lid][tid] = !s.launches[lid][tid];
  });

  const todayPosts = allPosts.filter(p=>p.d===todayStr).sort((a,b)=>(a.tm||'').localeCompare(b.tm||''));
  const weekPosts  = allPosts.filter(p=>{
    if(!p.d||p.d===todayStr) return false;
    const diff = Math.ceil((new Date(p.d+'T12:00:00')-now)/86400000);
    return diff>0 && diff<=7;
  }).sort((a,b)=>a.d.localeCompare(b.d));

  const LAUNCHES = [
    {id:'asesoria',      name:'Lanzamiento Asesoría 1:1',  price:'90€',    date:'2026-07-06', tasks:CM_ASESORIA_TASKS},
    {id:'acompanamiento',name:'Lanzamiento Acompañamiento', price:'625€',   date:'2026-10-20', tasks:CM_ACOMP_TASKS},
    {id:'blackfriday',   name:'Black Friday ofertas',       price:'varios', date:'2026-11-24', tasks:CM_BF_TASKS},
  ];
  ...
  const PostRow = ({p, compact=false}) => {
    const st = getStatus(p);
    const stC = {pending:C.inkSoft, creating:'#C4956A', scheduled:'#7BA2C2', manual:'#9B6FA8', published:C.published};
    const dd = p.ds || (p.d ? p.d.slice(8,10)+' '+_CM_MOS[+p.d.slice(5,7)-1] : '—');
    return (
      <div style={{background:C.card, borderRadius:10, padding:compact?'8px 12px':'11px 14px', marginBottom:5, border:`1px solid ${C.borderSoft}`, borderLeft:`3px solid ${pfDark(p.pf)}`, display:'flex', alignItems:'center', gap:10}}>
        <span style={{fontSize:9, fontWeight:700, color:pfDark(p.pf), background:pfBg(p.pf), padding:'2px 6px', borderRadius:3, flexShrink:0}}>{pfLabel(p.pf)}</span>
        {!compact && p.tm && <span style={{fontSize:10, color:C.inkSoft, flexShrink:0}}>{p.tm}</span>}
        {!compact && <span style={{fontSize:10, color:C.inkSoft, flexShrink:0}}>{p.t||''}</span>}
        {compact && <span style={{fontSize:10, color:C.accent, flexShrink:0}}>{dd}</span>}
        <span style={{flex:1, fontSize:compact?11:13, color:C.ink, overflow:'hidden', whiteSpace:'nowrap', textOverflow:'ellipsis'}}>{p.ti||p.su||'—'}</span>
        <select value={st} onChange={e=>setPostStatus(p,e.target.value)} onClick={e=>e.stopPropagation()} style={{fontSize:10, padding:'3px 6px', borderRadius:6, border:`1px solid ${C.border}`, background:C.bgSoft, color:stC[st]||C.inkSoft, fontFamily:'inherit', fontWeight:600, cursor:'pointer', flexShrink:0}}>
  ...
  return (
    <div style={{padding:'20px 20px 40px', maxWidth:880, margin:'0 auto'}}>
      <div style={{marginBottom:20}}>
      ...
      <div style={{display:'grid', gridTemplateColumns:'repeat(5,1fr)', gap:4, marginBottom:24, background:C.bgSoft, borderRadius:12, padding:4}}>
        {TABS.map(t=>(
          <button key={t.id} onClick={()=>setTab(t.id)} style={{padding:'9px 4px', border:'none', borderRadius:8, cursor:'pointer', fontFamily:'inherit', fontSize:11, fontWeight:500, background:tab===t.id?C.card:'transparent', color:tab===t.id?C.ink:C.inkSoft, boxShadow:tab===t.id?`0 1px 3px ${C.border}`:'none', transition:'all .15s'}}>
            <span style={{display:'block', fontSize:15, marginBottom:2}}>{t.emoji}</span>{t.label}
          </button>
        ))}
      </div>
        ...
        <div>
          <div style={{background:C.card, borderRadius:12, padding:'14px 16px', border:`1px solid ${C.borderSoft}`, marginBottom:4}}>
            <div style={{fontFamily:"'Fraunces', serif", fontSize:16, fontStyle:'italic', color:C.ink, marginBottom:2}}>
              {_CM_DAYS[now.getDay()]}, {now.getDate()} {_CM_MOS[now.getMonth()]}
            </div>
            <div style={{fontSize:12, color:C.inkSoft}}>{todayPosts.length>0?`${todayPosts.length} publicación${todayPosts.length>1?'es':''} hoy`:'sin publicaciones hoy'}</div>
          </div>

          {todayPosts.length>0 && <><SL>contenido de hoy</SL>{todayPosts.map(p=><PostRow key={`${p.pf}:${p.i}`} p={p}/>)}</>}

          <SL>⚡ acciones urgentes</SL>
          {CM_ACCIONES.filter(a=>a.pri==='alta').map(a=>{
            const done = !!(state.acciones?.[a.id]);
            return (
              <div key={a.id} onClick={()=>toggleAccion(a.id)} style={{background:C.card, borderRadius:10, padding:'12px 14px', marginBottom:5, border:`1px solid ${C.borderSoft}`, borderLeft:`3px solid ${done?C.published:C.accent}`, display:'flex', gap:10, cursor:'pointer', opacity:done?0.45:1}}>
          ...
          {weekPosts.length>0 && <><SL>esta semana</SL>{weekPosts.map(p=><PostRow key={`${p.pf}:${p.i}w`} p={p} compact/>)}</>}
        </div>
      )}
        ...
        <div>
          <div style={{display:'flex', gap:8, marginBottom:16, flexWrap:'wrap', alignItems:'center'}}>
            ...
            {['all','instagram','linkedin','substack','tiktok'].map(pf=>(
              <button key={pf} onClick={()=>setFilterPf(pf)} style={{padding:'5px 12px', borderRadius:8, border:'none', cursor:'pointer', fontFamily:'inherit', fontSize:11, fontWeight:500, background:filterPf===pf?(pf==='all'?C.ink:pfDark(pf)):C.bgSoft, color:filterPf===pf?'#fff':C.inkSoft}}>
                {pf==='all'?'Todas':pfLabel(pf)}
              </button>
            ))}
          </div>
              ...
              return (
                <div key={col.key} style={{minWidth:140}}>
                  ...
                  {colPosts.map(p=>{
                    const dd = p.ds||(p.d?p.d.slice(8,10)+' '+_CM_MOS[+p.d.slice(5,7)-1]:'—');
                    return (
                      <div key={`${p.pf}:${p.i}`} style={{background:C.card,borderRadius:8,padding:'8px 10px',marginBottom:6,border:`1px solid ${C.borderSoft}`,borderLeft:`3px solid ${pfDark(p.pf)}`}}>
                        <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:3}}>
                          <span style={{fontSize:9,fontWeight:700,color:pfDark(p.pf),background:pfBg(p.pf),padding:'1px 5px',borderRadius:3}}>{pfLabel(p.pf)}</span>
                        ...
                        <div style={{fontSize:11,color:C.ink,lineHeight:1.3,marginBottom:6}}>{(p.ti||p.su||'—').slice(0,48)}</div>
                        <div style={{display:'flex',gap:3,justifyContent:'flex-end'}}>
                          {ci>0&&<button onClick={()=>setPostStatus(p,COLS[ci-1].key)} style={{padding:'2px 6px',border:`1px solid ${C.border}`,borderRadius:4,background:'transparent',color:C.inkSoft,fontSize:10,cursor:'pointer'}}>←</button>}
                          {ci<COLS.length-1&&<button onClick={()=>setPostStatus(p,COLS[ci+1].key)} style={{padding:'2px 7px',border:'none',borderRadius:4,background:COLS[ci+1].color,color:'#fff',fontSize:10,cursor:'pointer',fontWeight:500}}>→</button>}
                        </div>
                      </div>
                    );
                  })}
      ...
      {tab==='acciones' && (
        <div>
          {['alta','media','baja'].map(pri=>{
            ...
            return (
              <div key={pri}>
                <SL><span style={{color:colors[pri]}}>→</span> {labels[pri]}</SL>
                {CM_ACCIONES.filter(a=>a.pri===pri).map(a=>{
                  const done = !!(state.acciones?.[a.id]);
                  return (
                    <div key={a.id} onClick={()=>toggleAccion(a.id)} style={{background:C.card,borderRadius:10,padding:'12px 14px',marginBottom:5,border:`1px solid ${C.borderSoft}`,display:'flex',gap:10,cursor:'pointer',opacity:done?0.45:1}}>
      ...
      {tab==='lanzamientos' && (
        <div>
          {LAUNCHES.map(launch=>{
            const done = launch.tasks.filter(t=>state.launches?.[launch.id]?.[t.id]).length;
            const total = launch.tasks.length;
            const pct = Math.round((done/total)*100);
            const daysLeft = Math.ceil((new Date(launch.date+'T12:00:00')-now)/86400000);
                  ...
                  {cats.map(cat=>(
                    <div key={cat} style={{marginBottom:12}}>
                      ...
                      {launch.tasks.filter(t=>t.cat===cat).map(t=>{
                        ...
                        return (
                          <div key={t.id} onClick={()=>toggleLaunchTask(launch.id,t.id)} style={{display:'flex',alignItems:'center',gap:10,padding:'8px 0',borderBottom:`1px solid ${C.borderSoft}`,cursor:'pointer',opacity:checked?0.45:1}}>
      ...
      {tab==='estrategia' && (
        <div>
          <div style={{display:'flex',gap:6,marginBottom:20,flexWrap:'wrap'}}>
            {[{id:'mayo',label:'🌱 Mayo'},{id:'junio',label:'🌿 Junio'},{id:'julio',label:'☀️ Julio'},{id:'resumen',label:'📋 Resumen'}].map(t=>(
              <button key={t.id} onClick={()=>setStratTab(t.id)} style={{padding:'7px 14px',border:`1px solid ${stratTab===t.id?C.accent:C.border}`,borderRadius:8,cursor:'pointer',fontFamily:'inherit',fontSize:12,fontWeight:500,background:stratTab===t.id?C.accent+'15':C.card,color:stratTab===t.id?C.accent:C.inkSoft}}>{t.label}</button>
            ))}
          </div>

          <div style={{background:C.card,borderRadius:12,padding:'14px 16px',border:`1px solid ${C.borderSoft}`,marginBottom:4}}>
            <div style={{fontSize:11,fontWeight:600,color:C.ink,marginBottom:6}}>Punto de partida</div>
            <div style={{fontSize:12,color:C.inkSoft,lineHeight:1.6}}>597 seguidoras · engagement real 3-5 personas · 0 ventas · asesorías sin preparar. <strong style={{color:C.ink}}>Un solo objetivo por mes.</strong></div>
          </div>

          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:8,marginBottom:16}}>
            {[{id:'mayo',e:'🌱',l:'Mayo',p:'construir audiencia con intención + primeras ventas ebook'},{id:'junio',e:'🌿',l:'Junio',p:'consolidar confianza + preparar asesorías en silencio'},{id:'julio',e:'☀️',l:'Julio',p:'decisión informada: lanzar o mover a septiembre'}].map(m=>(
              <div key={m.id} style={{background:C.bgSoft,borderRadius:10,padding:12,border:`1px solid ${stratTab===m.id?C.accent:C.borderSoft}`}}>
                <div style={{fontSize:12,fontWeight:700,color:C.accent,marginBottom:4}}>{m.e} {m.l.toUpperCase()}</div>
                <div style={{fontSize:11,color:C.inkSoft,lineHeight:1.5}}>{m.p}</div>
              </div>
            ))}
          </div>
          ...
          {stratTab==='mayo'&&(
            <div>
              <div style={{background:C.card,borderRadius:12,padding:'14px 16px',border:`1px solid ${C.borderSoft}`,marginBottom:4}}>
                <div style={{fontSize:11,fontWeight:600,color:C.ink,marginBottom:6}}>🔍 El diagnóstico real</div>
                ...
                <div style={{background:C.bgSoft,borderRadius:8,padding:12,fontSize:12,color:C.inkSoft,fontStyle:'italic'}}>"La estrategia original está bien diseñada para una audiencia que ya confía en ti. Aún no tienes esa audiencia. Hay que construirla primero."</div>
              </div>
              <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10,marginBottom:4}}>
                <div style={{background:C.bgSoft,borderRadius:10,padding:'14px 16px',textAlign:'center'}}>
                  ...
                  <div style={{fontSize:11,color:C.inkSoft}}>ventas ebook · objetivo mayo</div>
                </div>
                <div style={{background:C.bgSoft,borderRadius:10,padding:'14px 16px'}}>
                  <div style={{fontSize:11,fontWeight:600,color:C.ink,marginBottom:6}}>Topics de descubrimiento</div>
                  {['"Qué poner en tu LinkedIn si no tienes experiencia"','"Por qué no te llaman a entrevistas"','"Cómo explicar que acabas de terminar la carrera"','"El error del 90% de recién graduadas en su CV"'].map((t,i)=><div key={i} style={{fontSize:11,color:C.inkSoft,padding:'2px 0'}}>→ {t}</div>)}
                </div>
              </div>
              <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10}}>
                <div style={{background:C.card,borderRadius:10,padding:'12px 14px',border:`1px solid ${C.borderSoft}`}}>
                  <div style={{fontSize:10,fontWeight:700,color:C.published,marginBottom:8}}>✅ SE MANTIENE</div>
                  {['Inrō → hazlo (45 min)','Testimonio por DM → esta semana','CTA al ebook en checklist (15 min)','Secuencia 3 emails → práctica de voz'].map((t,i)=><div key={i} style={{fontSize:11,color:C.ink,padding:'4px 0',borderBottom:`1px solid ${C.borderSoft}`}}>✅ {t}</div>)}
                </div>
                <div style={{background:C.card,borderRadius:10,padding:'12px 14px',border:`1px solid ${C.borderSoft}`}}>
                  <div style={{fontSize:10,fontWeight:700,color:'#D85A6F',marginBottom:8}}>✗ SE QUITA</div>
                  {['Urgencia precio 26 mayo','Objetivo 5-10 ventas (genera ansiedad)'].map((t,i)=><div key={i} style={{fontSize:11,color:C.ink,padding:'4px 0',borderBottom:`1px solid ${C.borderSoft}`}}>✗ {t}</div>)}
                </div>
              </div>
            </div>
          )}
          ...
          {stratTab==='junio'&&(
            <div>
              {[{s:'Semana 1',l:'sembrar asesoría',d:'Posts que muestran que a veces el ebook no es suficiente.'},{s:'Semana 2',l:'educar sobre guía personal',d:'Qué es una sesión de orientación. Qué pasa durante. Qué cambia después.'},{s:'Semana 3',l:'testimonios + yoga day',d:'Prueba social del ebook + conexión con bienestar laboral.'},{s:'Semana 4',l:'pre-lanzamiento asesoría',d:'Lista de espera abierta. Stories de urgencia suave.'}].map((s,i)=>(
                <div key={i} style={{background:C.card,borderRadius:10,padding:'12px 14px',border:`1px solid ${C.borderSoft}`,marginBottom:6,display:'flex',gap:12}}>
                  <div style={{fontSize:10,fontWeight:700,color:C.accent,flexShrink:0,marginTop:2}}>{s.s}</div>
                  <div><div style={{fontSize:12,fontWeight:600,color:C.ink,marginBottom:3}}>{s.l}</div><div style={{fontSize:11,color:C.inkSoft}}>{s.d}</div></div>
                </div>
              ))}
            </div>
          )}
          ...
          {stratTab==='julio'&&(
            <div>
              <div style={{background:C.card,borderRadius:12,padding:'14px 16px',border:`1px solid ${C.borderSoft}`,marginBottom:4}}>
                <div style={{fontSize:12,color:C.inkSoft,lineHeight:1.7,marginBottom:12}}>El 6 de julio es el lanzamiento de la asesoría. La decisión se toma con los datos de mayo y junio.</div>
                <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10}}>
                  <div style={{background:C.bgSoft,borderRadius:8,padding:12}}>
                    ...
                    <div style={{fontSize:11,color:C.inkSoft,lineHeight:1.6}}>100+ emails · engagement subiendo · 3+ testimonios · Inrō funcionando</div>
                  </div>
                  <div style={{background:C.bgSoft,borderRadius:8,padding:12}}>
                    <div style={{fontSize:11,fontWeight:700,color:'#C4956A',marginBottom:6}}>Escenario B · mueves a septiembre</div>
                    <div style={{fontSize:11,color:C.inkSoft,lineHeight:1.6}}>Lista pequeña · poca prueba social · agosto para construir, septiembre para lanzar</div>
                  </div>
                </div>
              </div>
            </div>
          ...
          {stratTab==='resumen'&&(
            <div>
              <div style={{fontSize:11,fontWeight:600,color:C.ink,marginBottom:10}}>Marcas de referencia</div>
              {[{h:'@theglwguide',l:'guía visual + digital products · comunidad muy fiel antes de vender nada'},{h:'@theproductivitymethod',l:'educa todos los días · el producto es consecuencia natural del contenido'},{h:'@the.habittracker',l:'contenido de valor repetible · mismo problema · distintos ángulos cada semana'},{h:'@hausofplanner',l:'digital products + comunidad · venden con naturalidad porque dan primero'},{h:'@creatorcollege_',l:'educa sobre crear contenido · la confianza se construye post a post'}].map((b,i)=>(
                <div key={i} style={{background:C.card,borderRadius:10,padding:'10px 14px',marginBottom:5,border:`1px solid ${C.borderSoft}`,display:'flex',gap:12}}>
                  <span style={{fontSize:12,fontWeight:600,color:C.accent,flexShrink:0}}>{b.h}</span>
                  <span style={{fontSize:12,color:C.inkSoft,lineHeight:1.5}}>{b.l}</span>
                </div>
              ))}
            </div>
          )}
